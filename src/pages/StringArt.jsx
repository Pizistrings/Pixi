...
export default function StringArt() {
  ...
  const generateStringArt = useCallback(async () => {
    ...
    // Create working image (grayscale for each color)
    const workingImages = selectedColors.map(() => {
      ...
      for (let y = 0; y < size; y++) {
        for (let x = 0; x < size; x++) {
          ...
          arr[y * size + x] = 255 - gray; // Invert so dark = high value
        }
      }
      return arr;
    });
